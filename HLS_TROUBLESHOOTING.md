...
現在の Windows パッケージでは `GST_PLUGIN_FEATURE_RANK` を固定値で持たず、
`uxplay_gst_rank.ps1` が起動時に証跡付きで判定する。

判定根拠は 2 種類に分けている。

- ハードウェア根拠: GPU 名と同梱プラグイン DLL の有無
- ポリシー根拠: `-hls` 起動時だけ適用する HLS 用の安全側設定

生成された証跡は `uxplay-gst-rank-report.json` に保存される。

現在の主な自動ルール:

- `nvh264dec` / `nvh265dec`: NVIDIA GPU が見つからない PC では `:0`
- `wasapi2sink` / `wasapisink`: `-hls` 起動時のみ `:0`
- `hlsdemux`: `-hls` 起動時のみ `:0`

静的ケースは [tests/test_windows_gst_rank.ps1](tests/test_windows_gst_rank.ps1) で固定しており、
代表的な GPU 構成ごとの差分を CI で検証する。

ランチャーが内部で実行する診断は次のスクリプト:

```bat
powershell -NoProfile -ExecutionPolicy Bypass -File "%HERE%uxplay_gst_rank.ps1" -BundleRoot "%HERE%"
```
